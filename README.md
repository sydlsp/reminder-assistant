...
语音输入使用微信原生录音和 `speechToText` 云函数，不依赖小程序插件。请先在腾讯云语音识别控制台开通“一句话识别”，然后部署 `cloudfunctions/speechToText` 云函数。云函数会优先使用运行环境提供的临时腾讯云凭证；如果当前云开发环境没有提供，请在该云函数的环境变量中配置 `ASR_SECRET_ID` 和 `ASR_SECRET_KEY`。密钥只能配置在云函数环境变量中，不要写入代码、前端或提交到 Git。首次在真机使用时，微信会请求麦克风权限；开发者工具中的录音效果可能与真机不同，建议使用真机预览验证。
...
- `cloudfunctions/speechToText`：调用腾讯云一句话识别，将 60 秒内的语音转成文字。
